...
bash ~/.claude/kit/infra/scripts/worker-resume.sh "$RUNID" <N> <tier> <worktree> --base "$BASE" --attempt <A> --answer "Consult posted: read the newest **Consult** comment on #<N> and follow its decision." --env DATABASE_URL=postgres://...
```
