...
respawn** onto the same worktree — never `rm` (it deletes the worktree being recovered), and never
...
`claude attach orch-<runid>-issue-<N>` — attach for anything about code, so the code never enters the
